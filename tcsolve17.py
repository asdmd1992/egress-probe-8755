...
def _do_solve_inner(page, attempt_start=1):
    ...
    for attempt in range(attempt_start, attempt_start + MAX_CHALLENGES):
        ...
        # VERIFY_CAP env (default 1800) — request-never-fired cap, must outlive the delay.
        verify_cap = int(os.environ.get("TC_VERIFY_CAP", "2400"))
            ...
            while True:
                ...
                if len(captured["resp"]) > resp_n_before:
                    ...
                    if outcome and outcome != "verify:51":
                        break
                # tcsolve15: event-driven — new verify request may be delayed too;
                # wait until it is actually seen, then 60s grace
                new_ts = captured.get("verify_req_ts")
                if new_ts and new_ts != req_ts_before:
                    req_ts_seen2 = True
                if req_ts_seen2 and time.time() - captured.get("verify_req_ts", 0) > 60:
                    log("post-hybrid verify request seen but no response in 60s -> give up")
                    break
                if not req_ts_seen2 and time.time() - t0 > verify_cap:
                    log("post-hybrid: no new verify request within %ds -> give up" % verify_cap)
                    break
                time.sleep(1)
