...
def do_solve(page, attempt_start=1):
    ...
    for attempt in range(attempt_start, attempt_start + MAX_CHALLENGES):
        ...
        if not sprite_b or len(sprite_b) < 100:
            # tcsolve15: empty/short sprite body (seen in tcsolve14 attempt2) must
            # NOT crash the run — refresh instead of imdecode assert
            log("sprite dl empty/short (len=%d) -> refresh" % len(sprite_b or b""))
            if attempt < MAX_CHALLENGES:
                time.sleep(2)
                refresh_challenge(frame)
                time.sleep(3)
        ...
        verify_cap = int(os.environ.get("TC_VERIFY_CAP", "1800"))
        while True:
            if captured["req"]:
                solved = True
