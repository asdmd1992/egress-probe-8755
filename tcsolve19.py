#!/usr/bin/env python3
# tcsolve19: EVENT-DRIVEN verify-response wait (no fixed cap for the 2nd verify after hybrid confirm)
# fact 9595 (run 32923337159): bot gave up at 2400s cap; page JS fired verify at +2450.5s, ec=9 no ticket.
# backoff doubling 25/90/183/305/922/1806/2450s -> hard cap 6300s (=2450*2.5) + fail-refresh loop (ec=9 ->
# re-handle hybrid if the page re-prompts hybrid, else outer loop refreshes and re-solves next challenge).
"""
tcsolve16.py — Tencent Cloud password-recovery chain solver v16 (captcha -> reset -> login).
...
def click_confirm_human(page, frame):
    """tcsolve17: human-trajectory (M1-style trusted CDP mouse) click on #verifyBtn.
    Hypothesis: Playwright synthetic el.click() on the CONFIRM button is fingerprinted as
    automation -> Tencent JS defers the cap_union_new_verify submission with increasing
    backoff (click->verify delay: 25s / 90s / 183s / 305s / 922s / 1806s across
    tcsolve10..16 runs, doubling each time). A trusted human-like click may take the
    normal path. Falls back to click_confirm()."""
    sel_list = ["#verifyBtn", "#embedVerifyBtn", "button[id*='verify']", "[class*='verifyButton']"]
    target = None
    for sel in sel_list:
        try:
    ...
    if not target:
        try:
            ...
            pass
    if not target:
        log("confirm_human: no button found -> fallback click_confirm")
        return click_confirm(frame)
    sel, el, bb = target
    ...
    try:
        ...
        cxp = (sx + cx) / 2 + random.uniform(-50, 50)
        cyp = (sy + cy) / 2 + random.uniform(-30, 30)
        for i in range(1, 31):
            t = i / 30.0
            t = t * t * (3 - 2 * t)
            mt = 1 - t
            x = mt * mt * sx + 2 * mt * t * cxp + t * t * cx + random.uniform(-1.5, 1.5)
            y = mt * mt * sy + 2 * mt * t * cyp + t * t * cy + random.uniform(-1.5, 1.5)
            page.mouse.move(x, y)
            time.sleep(random.uniform(0.004, 0.014))
        time.sleep(random.uniform(0.15, 0.4))
        page.mouse.down()
        time.sleep(random.uniform(0.1, 0.25))
        page.mouse.move(cx + random.uniform(-2, 2), cy + random.uniform(-2, 2), steps=2)
        time.sleep(random.uniform(0.08, 0.18))
        page.mouse.up()
        log("confirm_human: M1 trusted click on", sel, "at", (round(cx, 1), round(cy, 1)))
        return True
    except Exception as e:
        log("confirm_human M1 err:", str(e)[:120], "-> fallback el.click")
        return click_confirm(frame)
...
def solve_hybrid_click(page, frame, captured, attempt, EVID):
    ...
    try:
        ...
        if ok:
            ...
            click_confirm_human(page, frame)
            out["confirm"] = True
    except Exception as e:
        out["err"] = str(e)[:150]
    return out
...
def do_solve(page, attempt_start=1):
    """tcsolve17 wrapper: any exception inside _do_solve_inner still saves evidence
    (run 32920318771 failed with Page.screenshot crash -> NO artifact uploaded ->
    evidence lost; upload-artifact step has no if: always())."""
    try:
        return _do_solve_inner(page, attempt_start)
    except Exception as e:
        import traceback as _tb
        log("do_solve EXCEPTION:", _tb.format_exc()[-600:])
        try:
            save_json("captured.json", {"req": [], "resp": [], "verify_req": None})
            with open(os.path.join(EVID, "dryrun_summary.json"), "w") as f:
                json.dump({"error": str(e)[:400], "verify_outcomes": [],
                           "sendRecoverEmail_fired": False}, f, ensure_ascii=False, indent=1)
            shot(page, "dryrun_error.png")
        except Exception:
            pass
        return False, attempt_start


def _do_solve_inner(page, attempt_start=1):
    ...
    def on_resp(r):
        ...
        if "cap_union_prehandle" in r.url or "cap_union_new_getcapbysig" in r.url:
            ...
            captured["gc_count"] = captured.get("gc_count", 0) + 1
            if not obj:
                captured["gc_empty"] = captured.get("gc_empty", 0) + 1
            else:
    ...
    if USE_CLIP:
        try:
            ...
            clip = None
    # tcsolve19: phase-level wall budget. With event-driven 6300s waits, a couple of
    # fail-refresh cycles per phase can eat hours; 180min keeps the job under the 330min
    # workflow timeout (two phases). Env TC_PHASE_BUDGET to override.
    phase_budget = int(os.environ.get("TC_PHASE_BUDGET", "10800"))
    phase_t0 = time.time()
    for attempt in range(attempt_start, attempt_start + MAX_CHALLENGES):
        if time.time() - phase_t0 > phase_budget:
            log("PHASE BUDGET exceeded (%ds) -> stop phase" % phase_budget)
            break
        human_delay(1.2, 2.8)
        ...
        try:
            ...
            if attempt < MAX_CHALLENGES:
                ...
                continue
            break
        if not sprite_b or len(sprite_b) < 100:
            # tcsolve17: empty/short sprite body -> retry download 3x with random
            # backoff BEFORE refresh (tcsolve16: 3 consecutive empty downloads after a
            # verify:51 -> likely transient/sess-race, immediate refresh did not help)
            for _r in range(3):
                if sprite_b and len(sprite_b) >= 100:
                    break
                ...
                try:
                    ...
                    sprite_b = b""
                log("sprite dl retry#%d len=%d" % (_r + 1, len(sprite_b or b"")))
            if not sprite_b or len(sprite_b) < 100:
                log("sprite dl still empty/short (len=%d) -> refresh" % len(sprite_b or b""))
                if attempt < MAX_CHALLENGES:
                    time.sleep(2)
                    refresh_challenge(frame)
                    time.sleep(3)
        ...
        try:
            ...
            log("sprite imdecode crash:", str(e)[:120], "-> refresh")
            if attempt < MAX_CHALLENGES:
                time.sleep(2)
                refresh_challenge(frame)
                time.sleep(3)
        ...
        outcome = None
        t0 = time.time()
        resp_n = len(captured["resp"])   # tcsolve15: only consume responses NEW to this attempt
        req_ts_seen = False
        req_ts_base = captured.get("verify_req_ts")  # tcsolve16: per-attempt baseline —
        # a STALE verify_req_ts from a previous attempt must NOT count (fact 9493: without
        # this, attempt 2+ would see the old ts, judge "seen but >120s" and give up at once).
        # tcsolve16: event-driven wait — the verify REQUEST delay is DYNAMIC and rising:
        # 183s (tcsolve12) / 305s (tcsolve13) / 922-935s (tcsolve15 run 32917866192, where
        # the 900s cap fired 2ms BEFORE the request went out, verify:51 never consumed).
        # So: wait until a NEW request is actually seen, then grant 120s grace.
        # VERIFY_CAP env (default 6300) — request-never-fired cap, must outlive the delay.
        # tcsolve19 (fact 9597): backoff doubling continues across runs (25/90/183/305/922/
        # 1806/2450s); 2450s already defeated the 2400s cap by 50s — the SAME cap also has to
        # outlive the 1st-verify delay, so default 2400 -> 6300 (=2450*2.5) for both waits.
        verify_cap = int(os.environ.get("TC_VERIFY_CAP", "6300"))
        # tcsolve17: diagnostics every 15s + silent-JS human re-click of #verifyBtn.
        # Evidence (run 32920318771): sprite dl/classify/click/confirm ALL fast (<7s);
        # the ENTIRE 1806s delay sits between #verifyBtn click and the page JS firing
        # cap_union_new_verify; delay doubles per run (25/90/183/305/922/1806s) =>
        # Tencent JS is deferring the verify submission with increasing backoff. If the
        # JS is completely silent (no verify req AND no getcapbysig/prehandle activity)
        # after 90s, the first click may not have been accepted -> re-click human-style.
        last_diag = time.time()
        last_gc = captured.get("gc_count", 0)
        retry_left = 2
        while True:
            if captured["req"]:
                solved = True
            ...
            if outcome:
                break
            cur_ts = captured.get("verify_req_ts")
            if cur_ts and cur_ts != req_ts_base:
                req_ts_seen = True
            if req_ts_seen and time.time() - captured["verify_req_ts"] > 120:
                log("verify request seen but no response in 120s -> give up this attempt")
                break
            if not req_ts_seen and time.time() - t0 > verify_cap:
                log("verify request never fired within %ds -> give up this attempt" % verify_cap)
                break
            now = time.time()
            if now - last_diag >= 15:
                last_diag = now
                ...
                try:
                    ...
                    _alive = False
                log("wait diag: +%.0fs req_seen=%s verify_req_count=%d gc=%d gc_empty=%d frame_alive=%s"
                    % (now - t0, req_ts_seen, captured.get("verify_req_count", 0),
                       captured.get("gc_count", 0), captured.get("gc_empty", 0), _alive))
            if (not req_ts_seen) and (now - t0 > 90) and retry_left > 0 \
                    and captured.get("gc_count", 0) == last_gc:
                retry_left -= 1
                log("JS silent for 90s (no verify, no gc activity) -> human re-click #verifyBtn, retries left=%d" % retry_left)
                try:
                    _ok = click_confirm_human(page, frame)
                    log("re-click #verifyBtn:", _ok)
                except Exception as _e:
                    log("re-click err:", str(_e)[:120])
                last_gc = captured.get("gc_count", 0)
            time.sleep(1)
        ...
        if outcome and outcome.startswith("verify:51"):
            ...
            # tcsolve19 (fact 9595): the 2nd verify (after hybrid confirm) is ALSO backoff-delayed
            # (it fired at +2450.5s in run 32923337159 — 50s AFTER the 2400s fixed cap gave up,
            # captured ec=9, never consumed). So: pure event-driven wait for the verify REQUEST,
            # 120s grace for its response, and a single hard cap (default 6300s = observed 2450 * 2.5)
            # purely to avoid an infinite hang. No 60s early-exit.
            resp_n_before = len(captured["resp"])
            req_ts_before = captured.get("verify_req_ts")
            t0 = time.time()
            req_ts_seen2 = False
            post_hybrid_cap = int(os.environ.get("TC_VERIFY_CAP", "6300"))
            last_diag = time.time()
            while True:
                if captured["req"]:
                    solved = True
                ...
                if len(captured["resp"]) > resp_n_before:
                    ...
                    if outcome and outcome != "verify:51":
                        break
                    # ec=51 again: page re-prompted hybrid — consume it, re-handle, wait for
                    # the NEXT verify request again (continue the loop).
                    resp_n_before = len(captured["resp"])
                    req_ts_before = captured.get("verify_req_ts")
                    t0 = time.time()
                    req_ts_seen2 = False
                    log("post-hybrid verify:51 (re-prompted) -> re-handle hybrid")
                    hybrid = handle_hybrid(page, frame, captured, attempt, EVID)
                    captured["hybrid"] = hybrid
                    log("hybrid:", json.dumps(hybrid, ensure_ascii=False)[:300])
                new_ts = captured.get("verify_req_ts")
                if new_ts and new_ts != req_ts_before:
                    req_ts_seen2 = True
                if req_ts_seen2 and time.time() - captured.get("verify_req_ts", 0) > 120:
                    log("post-hybrid verify request seen but no response in 120s -> give up")
                    break
                if not req_ts_seen2 and time.time() - t0 > post_hybrid_cap:
                    log("post-hybrid: verify request never fired within %ds (hard cap) -> give up" % post_hybrid_cap)
                    break
                now = time.time()
                if now - last_diag >= 60:
                    last_diag = now
                    ...
                    try:
                        ...
                        _alive = False
                    log("post-hybrid wait diag: +%.0fs req_seen=%s verify_req_count=%d gc=%d frame_alive=%s"
                        % (now - t0, req_ts_seen2, captured.get("verify_req_count", 0),
                           captured.get("gc_count", 0), _alive))
                time.sleep(1)
            log("post-hybrid outcome:", outcome)
        if outcome and outcome.startswith("verify:9") and not DRYRUN:
            # tcsolve19: failRefresh in LIVE mode — the page keeps a (possibly new) challenge.
            # If it re-prompted hybrid, handle it here so the 2nd-verify wait below covers it;
            # otherwise fall through to the failed-attempt refresh + next-attempt re-solve.
            try:
                body_txt9 = frame.inner_text("body")
            except Exception:
                body_txt9 = ""
            markers9 = {}
            try:
                markers9 = {"slideBg": len(frame.query_selector_all(".slideBg")),
                            "verifyBtn": len(frame.query_selector_all("#verifyBtn")),
                            "tc-drag": len(frame.query_selector_all(".tc-drag"))}
            except Exception:
                pass
            if markers9.get("slideBg", 0) > 0:
                log("failRefresh: page still hybrid -> handle_hybrid, then wait for verify response")
                hybrid = handle_hybrid(page, frame, captured, attempt, EVID)
                captured["hybrid"] = hybrid
                log("hybrid:", json.dumps(hybrid, ensure_ascii=False)[:300])
                resp_n_before = len(captured["resp"])
                ...
                while True:
                    ...
                    if new_ts and new_ts != req_ts_before:
                        req_ts_seen2 = True
                    if req_ts_seen2 and time.time() - captured.get("verify_req_ts", 0) > 120:
                        log("post-ec9: request seen but no response in 120s -> give up")
                        break
                    if not req_ts_seen2 and time.time() - t0 > post_hybrid_cap:
                        log("post-ec9: verify request never fired within %ds -> give up" % post_hybrid_cap)
                        break
                    time.sleep(1)
                log("post-ec9 outcome:", outcome)
            else:
                log("failRefresh: fresh click challenge expected -> failed-attempt refresh path")
        if outcome and outcome.startswith("verify:0"):
            log("solve: captcha verified (errorCode 0) -> ticket issued")
            break
        # tcsolve19 (fact 9595): DRYRUN must NOT stop on a verify:9 that the event-driven loop
        # timed out waiting for — the fail-refresh cycle (new challenge -> re-solve) IS the
        # thing under test. Stop only on real success (verify:0 / sendRecoverEmail).
        if DRYRUN and outcome and (outcome == "sendRecoverEmail" or outcome.startswith("verify:0")):
            log("dryrun: success outcome -> stopping")
            break
