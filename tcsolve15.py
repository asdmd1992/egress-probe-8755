...
def do_solve(page, attempt_start=1):
    ...
    for attempt in range(attempt_start, attempt_start + MAX_CHALLENGES):
        ...
        req_ts_seen = False
        # tcsolve15: event-driven wait — the verify REQUEST delay is dynamic
        # (183s tcsolve12 / 305s tcsolve13 / ~600s tcsolve14); a hard window
        # ALWAYS loses the race when the request fires right at the boundary.
        # So: wait until the request is actually seen, then grant the response
        # a 120s grace. 900s safety cap only if the request never fires.
        while True:
            if captured["req"]:
                solved = True
            ...
            if req_ts_seen and time.time() - captured["verify_req_ts"] > 120:
                ...
                break
            if not req_ts_seen and time.time() - t0 > 900:
                log("verify request never fired within 900s -> give up this attempt")
                break
            time.sleep(1)
        ...
        if outcome and outcome.startswith("verify:51"):
            ...
            while True:
                ...
                if req_ts_seen2 and time.time() - captured.get("verify_req_ts", 0) > 60:
                    ...
                    break
                if not req_ts_seen2 and time.time() - t0 > 900:
                    log("post-hybrid: no new verify request within 900s -> give up")
                    break
                time.sleep(1)
