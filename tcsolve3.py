...
def click_tile(page, frame, tile_idx):
    ...
    then verify .tc-click-mark registered (dy-ele ClickEl marks selection); if 0 marks ->
    page.mouse fallback at absolute coords. Returns True only if a mark registered."""
    try:
        box = frame.locator("#slideBg").bounding_box()
    except Exception:
        box = None
    if not box:
        log("click_tile: no slideBg box")
        return False
    col = (tile_idx - 1) % 3
    row = (tile_idx - 1) // 3
    ...
    while time.time() - t0 < 6:
        marks = count_marks(page, frame)
        if marks > 0:
            break
        time.sleep(0.4)
    if marks == 0:
        # locator click did not register -> mouse fallback at absolute coords
        try:
            page.mouse.move(absx, absy, steps=8)
            time.sleep(random.uniform(0.15, 0.4))
            page.mouse.click(absx, absy)
            ok = True
        except Exception:
            pass
        t1 = time.time()
        while time.time() - t1 < 4:
            ...
            if marks > 0:
                break
            time.sleep(0.4)
    log("clicked tile", tile_idx, "at", round(absx, 1), round(absy, 1),
        "marks:", marks, "via", "locator" if ok else "FAIL")
    return marks > 0
...
def do_solve(page):
    ...
    for attempt in range(1, MAX_CHALLENGES + 1):
        ...
        # tile selection: text challenges (包含文字) -> refresh immediately (OCR proven
        # useless on Tencent styled font: run3/run4 all-miss); object challenges -> classifier
        tile_idx = 0
        detail = []
        ...
        if not ok_click:
            ...
            break
        click_confirm(frame)
        # wait for outcome: verify response or sendRecoverEmail (30s window)
        t0 = time.time()
        while time.time() - t0 < 30:
            if captured["req"]:
                solved = True
                break
            if any("cap_union_new_verify" in x.get("url", "") for x in captured["resp"]):
                break
            time.sleep(1)
        if solved:
