        ...
        try:
            ...
            if el:
                ...
                if vis and bb:
                    ...
                    log("refresh url changed:", _su_before != _su_after)
                    return True
        except Exception as e:
...
def handle_hybrid(page, frame, captured, attempt, EVID):
    """errorCode=51 (verifyHybrid): point-click phase accepted but server wants hybrid
    (slider) verification. Dump UI state, look for slider elements in any frame,
    try to drag if present. Returns diagnostic dict."""
    out = {"attempt": attempt, "ts": round(time.time(), 2)}
    time.sleep(2.5)
    try:
        out["frames"] = [f.url[:160] for f in page.frames]
    except Exception as e:
        out["frames_err"] = str(e)[:120]
    try:
        html = frame.content()
        out["html_len"] = len(html)
        for marker in ("tc-drag", "drag-btn", "slideBtn", "slider", "handle", "embedVerifyBtn", "verifyBtn"):
    ...
    slider = None
    sels = ("#slideBtn", ".tc-drag-btn", ".drag_btn", "[class*='tc-drag'] button", "[class*='drag-btn']",
            "[class*='slider'] [class*='btn']", "[class*='handle']", "button[class*='drag']")
        ...
        for sel in sels:
            try:
                ...
                if bb and bb["width"] > 10 and bb["height"] > 10:
                    slider = {"frame": fr.url[:120], "sel": sel,
                              "box": {k: round(v, 1) for k, v in bb.items()}}
                    break
            except Exception:
                continue
        if slider:
    ...
    if not slider:
        out["slider"] = None
        out["conclusion"] = "no slider element found in any frame; UI unchanged after 51"
        try:
    ...
    try:
        ...
        hx = bx["x"] + bx["width"] / 2
        hy = bx["y"] + bx["height"] / 2
        fr = [f for f in page.frames if f.url[:120] == slider["frame"]][0]
        bg = None
        for sel2 in ("#slideBg", ".tc-drag-bg", "[class*='slideBg']", "img[class*='bg']"):
            try:
                ...
                continue
        if bg:
            tx = bg["x"] + bg["width"] - bx["width"] / 2 - 2
        else:
            tx = bx["x"] + 260
        out["drag_from"] = [round(hx, 1), round(hy, 1)]
        out["drag_to"] = [round(tx, 1), round(hy, 1)]
        page.mouse.move(hx, hy, steps=5)
        time.sleep(random.uniform(0.15, 0.35))
        page.mouse.down()
        steps = max(20, int(abs(tx - hx) / 6))
        for i in range(1, steps + 1):
            t = i / float(steps)
            t = t * t * (3 - 2 * t)
            page.mouse.move(hx + (tx - hx) * t, hy + random.uniform(-1.2, 1.2))
