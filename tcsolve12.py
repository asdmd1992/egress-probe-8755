...
def do_solve(page, attempt_start=1):
    ...
    for attempt in range(attempt_start, attempt_start + MAX_CHALLENGES):
        ...
        # tcsolve12 (fact 9480 analysis): the cap_union_new_verify REQUEST itself fires
        # only ~25-90s after the #verifyBtn click on this egress (runs 32913585817:+25s,
        # 32914238062:+90.0s; response ~3s later). The old 90s window expired right
        # before the request went out => outcome=None, verify:51 never handled.
        # => hard window 180s + smart exit 60s after the request is actually seen.
        outcome = None
        t0 = time.time()
        while time.time() - t0 < 180:
            if captured["req"]:
            ...
            if outcome:
                break
            if captured.get("verify_req_ts") and time.time() - captured["verify_req_ts"] > 60:
                log("verify request seen but no response in 60s -> give up this attempt")
                break
            time.sleep(1)
