...
def count_marks(page, frame):
    """count click-mark elements (tc class + any [class*='mark']) in frame and main doc.
    returns dict {tc, any} so a wrong class-name hypothesis is visible."""
    out = {"tc": 0, "any": 0}
...
def click_tile(page, frame, tile_idx):
    ...
    def mark_els():
        ...
        out = []
        try:
            out += frame.evaluate("""Array.from(document.querySelectorAll("[class*='mark']")).slice(0,10).map(e =>
    ...
    # M1: trusted CDP mouse click at main-page coords
    human_delay(0.5, 1.1)
    try:
    ...
    log("click_tile: M1 marks=", m, "evt=", diag["m1_evt"])
    if m["tc"] > 0:
