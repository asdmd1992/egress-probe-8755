...
OBJ = {
 ...
 "羊":"sheep","猪":"pig","熊":"bear","老虎":"tiger","狮子":"lion","大象":"elephant","猴子":"monkey",
...
def count_marks(page, frame):
    """count .tc-click-mark elements in frame and main doc (operationDom may live in either)"""
    n = 0
    for scope in (frame, page):
        try:
            n += len(scope.query_selector_all(".tc-click-mark"))
        except Exception:
            pass
    return n
...
def click_tile(page, frame, tile_idx):
    """v4 interaction ladder: trusted CDP mouse -> JS dispatchEvent -> jQuery trigger.
    Returns (ok, diag) where ok=True only if >=1 .tc-click-mark registered (fact 9465:
    all 3 methods failed on sandbox; here each method + window.__errs trap is recorded
    so the runner run tells us WHICH mechanism works and whether the handler throws)."""
    diag = {}
    try:
    ...
    # what element is actually at the tile center (frame-local hit test)
    try:
    ...
    # Method 1: trusted CDP mouse click at absolute coords
    human_delay(0.5, 1.1)
    try:
        page.mouse.move(absx, absy, steps=7)
    ...
    diag["m1_marks"] = m1
    if m1 > 0:
        log("click_tile: M1 (trusted mouse) registered", m1, "marks")
        return True, diag
    diag["m1_errs"] = errs()
    log("click_tile: M1 0 marks; errs:", diag["m1_errs"])

    # Method 2: JS dispatchEvent on slideBg (bubbles) with clientX/Y
    try:
        r = frame.evaluate("""(pt) => {
    ...
    diag["m2_marks"] = m2
    if m2 > 0:
        log("click_tile: M2 (JS dispatch) registered", m2, "marks")
        return True, diag
    diag["m2_errs"] = errs()
    log("click_tile: M2 0 marks; errs:", diag["m2_errs"])

    # Method 3: jQuery trigger on slideBg with explicit offsetX/Y
    try:
        r = frame.evaluate("""(pt) => {
            const sb = document.getElementById('slideBg');
    ...
    diag["m3_marks"] = m3
    if m3 > 0:
        log("click_tile: M3 (jQuery trigger) registered", m3, "marks")
        return True, diag
    diag["m3_errs"] = errs()
    log("click_tile: M3 0 marks; errs:", diag["m3_errs"])

    # Method 4: direct dispatch on the tc-imgarea container (parent of slideBg)
    try:
        r = frame.evaluate("""(pt) => {
    ...
    diag["m4_errs"] = errs()
    if m4 > 0:
        log("click_tile: M4 registered", m4, "marks")
        return True, diag
    log("click_tile: all methods 0 marks; final errs:", diag["m4_errs"])
    return False, diag
...
def do_solve(page):
    ...
    for attempt in range(1, MAX_CHALLENGES + 1):
        ...
        if not ok_click:
            log("tile click did NOT register (0 marks via all methods) -> refresh")
            if attempt < MAX_CHALLENGES:
                time.sleep(2)
                refresh_challenge(frame)
                time.sleep(3)
